...
public class SensorDatabase {
	...
	/*
	 * init: This function opens up the socket and initializes
	 */
	public void init() throws UnknownHostException, IOException {

		// Local Variables
		String START = "start\n"; // String that signals the DataGen server to
									// begin sending data
		...
		System.out
				.println("Configuring Sensor Database to support the following metrics: ");
		...
		// Closing the socket is a signal to the DataGen server that you don't
		// want any more data.
		clientSocket.close();
	...
	/*
	 * readFromDataGen: This function reads from port 6789 and stores data in
	...
	private void readFromDataGen() throws IOException {
		...
		System.out
				.println("Reading data for " + SECONDS_TO_READ + " seconds..");
	...
	/*
	 * getSummary: this function prints the details about the sensor database on
	 * System.out. Details about metrics stored, unit of all metrics, total
	...
	public void getSummary() {
		...
		// 2 is multiplied because data is stored in a map & a list for each
		// metric
		...
		System.out.println("Data worth " + SECONDS_TO_READ
				+ " seconds collected");

		System.out.println("Time span of Data Collected:");
		for (int i = 0; i < this.metrics.size(); i++) {
			System.out.println(this.metrics.get(i).getName() + ": "
					+ this.metrics.get(i).getStartTime() + " - "
		...
		for (int i = 0; i < this.metrics.size(); i++) {
			System.out.println(this.metrics.get(i).getName() + ": "
					+ this.metrics.get(i).getMax() + " - "
					+ this.metrics.get(i).getMin()
					+ this.metrics.get(i).getUnit());
		}
	...
	/*
	 * firstOccurance: This function finds the first occurance of the value of a
	 * given metric that is read from the 'Data Generator'
	 */
	...
	/*
	 * nextOccurance: this function prints the time when a given metric value is
	 * read. Every time the call is made, the function returns the next time
	 * when the metric value occured, if the value had occured multiple times.
	 */
	public boolean nextOccurance(String metric, double value) {
		...
		System.out
				.println("The metric used is not present the Sensor Database");
	...
	/*
	 * Main function to test the methods provided by the class. The main
	 * function is an example as to how the API can be used.
	 */
	...
	public static void main(String[] args) throws UnknownHostException,
		...
		while (true) {
			...
			case "r":
				...
				startTime = new Time(Integer.parseInt(starttimeSeperated[0]),
						Integer.parseInt(starttimeSeperated[1]),
						Integer.parseInt(starttimeSeperated[2]));
				endTime = new Time(Integer.parseInt(starttimeSeperated[0]),
						Integer.parseInt(endtimeSeperated[1]),
						Integer.parseInt(endtimeSeperated[2]));
			...
			case "s":
				...
				Time firstOccur = sdb.firstOccurance(metricinput,
						Double.parseDouble(metricvalue));

				if (firstOccur != null) {
					System.out.println("Time of first occuance of value "
							+ metricvalue + " for metric " + metricinput
							+ " :\n" + firstOccur.toString());
				} else {
